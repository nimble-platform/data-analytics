...
The data stored in Elasticsearch will by default be persisted after container reboot but not after container removal.

In order to persist Elasticsearch data even after removing the Elasticsearch container, a volume is mounted on your Docker host. Update the `elasticsearch` service declaration to:
